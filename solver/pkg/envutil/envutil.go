...
// GetConditionalEnv gets an environment variable based on DEVNET flag
// If DEVNET=true, uses LOCAL_* version, otherwise uses regular version
func GetConditionalEnv(key, defaultValue string) string {
	isDevnet := os.Getenv("DEVNET")
...
// GetConditionalUint64 gets a uint64 environment variable based on DEVNET flag
// If DEVNET=true, uses LOCAL_* version with local defaults, otherwise uses regular version with live defaults
...
// GetConditionalInt64 gets an int64 environment variable based on DEVNET flag
// If DEVNET=true, uses LOCAL_* version with local defaults, otherwise uses regular version with live defaults
...
// Common environment variable getters with DEVNET logic
// These provide type-safe access to commonly used environment variables

// GetStarknetAliceAddress returns Alice's Starknet address based on DEVNET flag
...
// GetStarknetAlicePrivateKey returns Alice's Starknet private key based on DEVNET flag
func GetStarknetAlicePrivateKey() string {
	return GetConditionalAccountEnv("STARKNET_ALICE_PRIVATE_KEY")
}

// GetStarknetAlicePublicKey returns Alice's Starknet public key based on DEVNET flag
func GetStarknetAlicePublicKey() string {
	return GetConditionalAccountEnv("STARKNET_ALICE_PUBLIC_KEY")
}

// GetStarknetSolverAddress returns solver's Starknet address based on DEVNET flag
