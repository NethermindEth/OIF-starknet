...
func TestConditionalEnvironment(t *testing.T) {
	t.Run("DEVNET=true uses LOCAL_ variables", func(t *testing.T) {
		...
		// Test that LOCAL_ version is used when DEVNET=true
		result := envutil.GetConditionalEnv("ETHEREUM_RPC_URL", "default")
		assert.Equal(t, "http://localhost:8545", result)
	})

	t.Run("DEVNET=false uses regular variables", func(t *testing.T) {
		// Set up test environment
		...
		// Test that regular version is used when DEVNET=false
		result := envutil.GetConditionalEnv("ETHEREUM_RPC_URL", "default")
		assert.Equal(t, "https://eth-sepolia.g.alchemy.com/v2/test", result)
	})

	t.Run("DEVNET unset defaults to regular variables", func(t *testing.T) {
		// Set up test environment
		os.Unsetenv("DEVNET")
		t.Setenv("LOCAL_ETHEREUM_RPC_URL", "http://localhost:8545")
		t.Setenv("ETHEREUM_RPC_URL", "https://eth-sepolia.g.alchemy.com/v2/test")
		...
		// Test that regular version is used when DEVNET is unset
		result := envutil.GetConditionalEnv("ETHEREUM_RPC_URL", "default")
		assert.Equal(t, "https://eth-sepolia.g.alchemy.com/v2/test", result)
	})
